...
std::vector< String > lastMessages;
...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
  ...
  if( x == true )
  {
      int x = NUMITEMS( lastMessages );

      if( x == 8 )
      {
          lastMessages.erase( 0 );
          lastMessages.erase( 1 );
      }

      lastMessages.push_back( "Username: " + String( incomingMessage.userName ) );
      lastMessages.push_back( "Message: " + String( incomingMessage.message ) );

      for( int i = 0; i < lastMessages.size(); i += 2 )
      {
          Serial.print("Bytes received: ");
          Serial.println(len);
          Serial.print( lastMessages[ i ] );
          Serial.print( " | " );
          Serial.print( lastMessages[ i + 1 ] );
          Serial.println();
